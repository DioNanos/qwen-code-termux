...
describe('core root barrel flat-config integration', () => {
  // fork: the six lintText calls cold-parse the whole flat config; on a
  // slow shared host under vitest's parallel workers they exceed the
  // 30s suite default. Timeout budget only — the assertions are intact.
  it(
    'reports production self-imports (static, type-only, inline-type, and dynamic), ignores tests',
    { timeout: 120_000 },
    async () => {
